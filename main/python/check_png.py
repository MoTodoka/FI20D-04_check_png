...
def check_png(dateipfad, png_byte_order='big'):
    print("Analysiere {0}".format(dateipfad))
...
def ist_png_datei(byte_reader):
    # Image file header. The first eight bytes are always the same for an image with file format png
    ...
    if erste_acht_byte == erwartete_erste_acht_byte:
        ...
        print("Die ersten acht Byte sind {0}".format(erste_acht_byte))
        print("Die ersten acht Byte sollten {0} sein".format(erwartete_erste_acht_byte))
        return False
